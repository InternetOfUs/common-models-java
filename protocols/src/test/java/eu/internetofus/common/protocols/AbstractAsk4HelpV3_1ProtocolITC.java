 ...
 */
public abstract class AbstractAsk4HelpV3_1ProtocolITC extends AbstractDefaultProtocolITC {
   ...
   * {@inheritDoc}
   *
   * @return {@link DefaultProtocols#ASK_4_HELP_V3_1}
   */
  @Override
  protected DefaultProtocols getDefaultProtocolsToUse() {

    return DefaultProtocols.ASK_4_HELP_V3_1;
  }
  ...
  protected Task createTaskForProtocol() {
    ...
    taskToCreate.attributes = new JsonObject().put("domain", this.domain()).put("anonymous", this.anonymous())
        .put("subjectivity", this.subjectivity()).put("maxUsers", this.maxUsers()).put("maxAnswers", this.maxAnswers())
  ...
  /**
   * Return the domain for the new task.
   *
   * @return the new task domain.
   */
  public abstract String domain();

  /**
   * Return the anonymous for the new task.
   *
   * @return the new task anonymous.
   */
  public abstract boolean anonymous();

  /**
   * Return the subjectivity for the new task.
   *
   * @return the new task subjectivity.
   */
  ...
  /**
   * The number of question to answer messages that has to be received when the
   * task is created.
   *
   * @return the count of messages to receive when the task is created.
   */
  ...
  public void shouldCreateTask(final Vertx vertx, final VertxTestContext testContext) {
    ...
    final var maxMessages = this.initQuestionToAnswerMessage();
