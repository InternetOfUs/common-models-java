...
if [ -f /.dockerenv ]; then
   echo "You can not start the development environment inside a docker container"
	...
	docker run --rm --name wenet_common_dev -v /var/run/docker.sock:/var/run/docker.sock -v ${HOME}/.m2:/root/.m2  -v ${PWD}:/app -p 8000:5005 -it internetofus/common:dev /bin/bash
