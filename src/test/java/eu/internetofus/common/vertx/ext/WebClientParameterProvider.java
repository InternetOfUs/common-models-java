...
public class WebClientParameterProvider implements VertxExtensionParameterProvider<WebClient> {
  ...
  public WebClient newInstance(ExtensionContext extensionContext, ParameterContext parameterContext) {
    ...
    Objects.requireNonNull(scopedObject,
        "A Vertx instance must exist, try adding the Vertx parameter as the first method argument");
  ...
  public static Optional<WebClientOptions> getWebClientOptions(ExtensionContext context) {
    ...
    List<Field> webClientOptionsField = AnnotationUtils.findPublicAnnotatedFields(thisTestClass.get(),
        WebClientOptions.class, WebClientOptionsInject.class);
    if (webClientOptionsField.isEmpty()) {
      return Optional.empty();
    }
    return ReflectionUtils.tryToReadFieldValue(webClientOptionsField.get(0), context.getTestInstance().get())
